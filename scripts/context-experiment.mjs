...
if(!dryRun){
  ...
  const strategies=[['Full history',full],['Hierarchical',hierarchical]].sort(()=>Math.random()-.5);
  for(const [name,messages] of strategies){console.log(`Running ${name}…`);results.push(await callKimi(name,messages,token,model));}
}else{
  results=[{name:'Full history',answer:'Dry run: no API request made.',inputTokens:estimatedTokens(full),outputTokens:null,exactInputTokens:false,latencyMs:null},{name:'Hierarchical',answer:'Dry run: no API request made.',inputTokens:estimatedTokens(hierarchical),outputTokens:null,exactInputTokens:false,latencyMs:null}];
}
results.sort((a,b)=>a.name.localeCompare(b.name));const fullResult=results.find(result=>result.name==='Full history');const hierarchicalResult=results.find(result=>result.name==='Hierarchical');const saving=((fullResult.inputTokens-hierarchicalResult.inputTokens)/fullResult.inputTokens*100).toFixed(1);
